...
int flash(int a[], int r, int c, int len){
	a[r*len+c] = 0;
	int collect = 1;
	...
	for(int i=-1; i<2; i++){
		for(int j=-1; j<2; j++){
			...
			if(	newRow>=0  && newRow<len && newCol>=0 && newCol<len){
				...
				if(a[loc]>9) collect += flash(a, newRow, newCol, len);
			}
...
int main(int argc, char** argv){
		...
		for(int row=0; row<length; row++){
			for(int col=0; col<length; col++){
				if(current[row*length+col] == 10){
					collect += flash(current, row, col, length);
				}
			}
		}
	...
	std::cout << "Everyone flashed at: " << ctr << std::endl;

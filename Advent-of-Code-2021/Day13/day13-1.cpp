...
void foldPaper(std::vector<std::vector<int>>& plane, Limits& lim, bool dir, int val, int& pointCount){

	if(dir){ //y fold
		for(int y=1; y<lim.yMax-val+1; y++){
			for(int col=lim.xMin; col<lim.xMax+1; col++){
				if(plane[val+y][col]){
					if(plane[val-y][col]) pointCount--;
					else plane[val-y][col] = 1;
				}
			}
		...
		for(int row=lim.yMin; row<lim.yMax+1; row++){
			for(int x=1; x<lim.xMax-val+1; x++){
				if(plane[row][val+x]){
					if(plane[row][val-x]) pointCount--;
					else plane[row][val-x] = 1;
				}
			}
		}
...
int main(int argc, char** argv){
	...
	//Read in the first fold instruction
	std::getline(in, line);
	std::stringstream ss(line);
	std::getline(ss, input,'\r');
	...
	foldPaper(plane, lim, direction, stoi(value), pointCount);
